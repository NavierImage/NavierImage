...
def CalContourVolSize(fol_path):
    ...
    I_NEED_IT = ["Cortex_R", "Cortex_L", "External"] #여기에 있는 것들만 Volume 계산

    for idx in range(item_num):
        polygon_vol = np.zeros((ct_list[-1][1].InstanceNumber, ct_list[-1][1].Rows, ct_list[-1][1].Columns), dtype="float64")
        ctr_vol = np.zeros_like(polygon_vol)
        ...
        for jdx in range(max_slices):
            ...
            polygon_vol[z1][rr, cc] = 1
            draw_y, draw_x = draw.line(y2, x2, ctr_int_data[0][1], ctr_int_data[0][0])
            ctr_vol[z1][draw_y, draw_x] = 1
        num_vox = len(np.where(polygon_vol==1)[0])
        num_ctr_pix = len(np.where(ctr_vol==1)[0])

        #mm^3 -> cc로 변환
        vol_size = num_vox*scale_set[0]*scale_set[1]*scale_set[2]/1000 - (num_ctr_pix*scale_set[0]*scale_set[1]*scale_set[2]*1/2)/1000
...
CalContourVolSize(r"D:\test_이식외과")
